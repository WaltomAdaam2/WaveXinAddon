...
public final class WaveXinSettingsStoreBehaviorTest {
    public static void main(String[] args) throws Exception {
        ...
        NbtCompound migratedTag = StringNbtReader.readCompound(migrated);
        expect(!migratedTag.getBoolean("active", true), "migration must retain recorder activation state");
        expect("none".equals(migratedTag.getString("bind", "")), "migration must retain recorder bind");
        expect(migratedTag.getCompound("settings").orElseThrow().getInt("Container Threshold", 0) == 17,
