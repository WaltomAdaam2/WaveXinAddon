...
- `Normal Scan` scans outward in rings from its starting chunk, moves between targets, and can wait for chunk loading. Reaching a target or resume checkpoint now requires returning to the target chunk center, preventing manual input or other movement modules from drifting the scan route. It prints one concise message after each completed ring. Progress is written only after reaching a valid route checkpoint, and Restart fields are read-only live displays; if the module is disabled away from a valid checkpoint, the last valid checkpoint is preserved and a warn diagnostic is written. The Restart reset button clears saved Normal Scan restart data. `Start From Previous Scan` only controls restoring, not saving; saved data is bound to the server and dimension and validates the ring, route, and checkpoint before resume.
...
- Normal render defaults to 128 chunks, allows up to 256 chunks, and preloads 10 rings by default up to a limit of 20. `Render Completed Area` keeps completed visited chunks visible with the visited green color instead of dropping them when the preview ring moves forward.
