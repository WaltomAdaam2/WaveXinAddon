...
- Both scan modes share container recording: a chunk is recorded when its selected-container count reaches the threshold. Ordinary target-center correction is normal movement and does not write warn logs; `BaseFinderDebug` warn logs are reserved for missing player/world state, current-chunk waits, and other diagnostic states.
