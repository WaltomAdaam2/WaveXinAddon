...
- Search visits concentric chunk rings outward from the current player chunk. An inner-ring candidate can start the route as soon as it is found while background work continues; it does not wait for the entire range. Circular and square search radii both default to 2,000 blocks and can reach 500,000 blocks, although large ranges still increase background computation time.
