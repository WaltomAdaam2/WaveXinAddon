...
- 可选的 Xaero 临时路径点会在模块启用时创建 `Elytra Path`（别名 `EP`）；下界换算开启时使用换算后的 X/Z，Y 使用启用瞬间玩家高度。停用时仅尝试移除本次启用创建的同一对象；Xaero 缺失或不支持该操作只警告一次，不会中断路径飞行。
...
- `Rolling Radius (Chunks)` 是以玩家为中心的圆形工作半径，默认 5,000 区块、范围 8–100,000。后台使用单线程从中心向外处理 32×32 区块任务；发现首个可用候选后即可开始路线，不等待整个范围完成。玩家距当前工作圆边缘 100 格时，扫描中心推进到玩家位置，并复用本次游戏会话内的重叠任务结果。
- 滚动缓存只保存在内存中，并按世界实例、种子及生成版本隔离；模块关闭后再次开启可复用相同会话缓存，切换世界、种子或生成版本时不会串用。右上角独立进度提示按批次显示扫描状态、已完成/总区块数及百分比。当前范围完成或暂无候选时模块仍保持启用，等待玩家推进下一范围或后台产生新结果。
