...
- `Normal Scan` 从起始区块向外按 ring 扫描，在目标区块间移动并可等待区块加载；到达目标或恢复断点时必须回到目标区块中心，防止手动输入或其它移动模块让扫图路线偏离。每完成一个完整 ring 输出一次简洁进度。模块只会在到达合法路线检查点时写入进度，Restart 组内恢复字段为只读实时显示；停用时如果玩家不在合法检查点，会保留上一个有效检查点并写入 warn 诊断。Restart 组底部的重置按钮会清除已保存的普通扫描恢复数据。`Start From Previous Scan` 仅负责恢复，不再控制是否保存；保存数据会绑定服务器与维度，并在恢复前校验 ring、路线和检查点。
