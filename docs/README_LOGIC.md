...
- 公共聊天按可解析玩家名判断；`<玩家名> 消息` 和受限的 `玩家名: 消息` 会被视为 public，常见服务器冒号前缀如 `使用方式:`、`系统:`、`Warning:`、`Error:` 和 `Server:` 不会按 public 过滤。`Show Own Public Messages` 默认启用，会同时比较账户名和去格式/前缀后的显示名。
...
- 关闭 `Quick Swap` 后只使用副手、主手或快捷栏内的目标药水；临时切换到其他快捷栏槽位后会尝试恢复原槽位。找不到可用药水、切换失败、投掷被拒绝或恢复失败都会写入 warn 级游戏 log；`Notify` 仅决定是否额外在聊天栏按 WaveXin 警告格式提示。

### Base Finder

- `Normal Scan` 从起始区块向外按 ring 扫描，在目标区块间移动并可等待区块加载；到达目标或恢复断点时必须回到目标区块中心，防止手动输入或其它移动模块让扫图路线偏离。每完成一个完整 ring 输出一次简洁进度。关闭时会保存当前断点，`Start From Previous Scan` 可恢复 Normal Scan 的位置、ring 和路线。
- `Spiral Scan` 使用独立的螺旋路线、步长、段数和渲染设置；自动走路时会持续朝当前目标区块中心修正方向，偏到目标区块边缘时先拉回中心再进入下一段。可选冲刺、视角锁定和打开界面时暂停。它不复用 Normal Scan 的断点逻辑。
- 两种扫描共享容器记录：扫描到已选择容器数量达到阈值的区块时，记录坐标与容器数量。中心修正进入异常状态时会写入 `BaseFinderDebug` warn 日志，正常移动不刷 info。
