...
- `1.21.11`：Minecraft 1.21.11（默认分支）
- `1.21.1`：Minecraft 1.21.1

行为修复、纯逻辑、翻译、文档和测试先提交到 `1.21.11`，再 cherry-pick 到 `1.21.1`。版本 API 修复只留在对应分支。这样可以先获得一个真实可启动的 1.21.1 版本，不需要在移植过程中同时重排全部目录。
